...
#            self.autoCorFittingButton.place(x=lv_x+220,y=lv_y)
    ...
    def readFiles(self):
        ...
        self.particles = list()
        self.files = [i for i in self.dn.glob('*.{}'.format('csv'))]
...
#                    self.particles[-1].MSAD = [float(i) for i in row[1:] if i != '']
    ...
    def readData2(self,f):
        '''Reads the data from the tracking file,
        according to v. 1.6.2.
        I won't read angles or MSAD, they are calculated in this script.
        The MSAD information from the tracking file is wrong.'''
        
        '''THIS PART IS TAILOR-MADE TO READ THE DATA FROM DONGPYO'''
        ...
        self.dn = self.dn + '\\' + sheet_names[i]
        
        if not os.path.exists(self.dn):
            os.mkdir(self.dn)
...
#        for i in range(len(sheet_names)):
        ...
        for j in range(sheet.ncols):
            ...
            if 'article' in sheet.cell_value(0,j):
                
                p = Particle()
                p.particleLabel = sheet.cell_value(0,j)
                p.fileName = f+'_'+sheetName
                print([sheet.cell_value(k,j) for k in range(2,sheet.nrows) if k != ''])
                p.time = [float(sheet.cell_value(k,j)) for k in range(2,sheet.nrows) if k != '']
                if p.time[0] != 0:
                    p.time = [p.time[n]-p.time[0] for n in range(len(p.time))]
                p.X = [float(sheet.cell_value(k,j+1)) for k in range(2,sheet.nrows) if k != '']
                p.Y = [float(sheet.cell_value(k,j+2)) for k in range(2,sheet.nrows) if k != '']
                p.timeD = p.time[1:]
                print(p.time)
                p.FPS = 1/(p.timeD[0])
...
#                    self.particles[-1].autoCorstd = [float(i) for i in row[1:] if i != '']
    ...
    def checkParticleValidity2(self,p):
        #A particle is only valid if it has X, Y, MSMD, timdD and time information
        #Instantaneous velocity is note necessary
#        if (not p.X) or (not p.Y) or (not p.MSD) or (not p.timeD) or (not p.time):
